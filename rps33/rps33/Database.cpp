...
std::vector<std::string> Database::getUserArrays(const std::string& login) {
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        if (date) {
            row += reinterpret_cast<const char*>(date);
        }
        row += " | ";
        ...
        if (original) {
            row += "Исходный: ";
            row += reinterpret_cast<const char*>(original);
        }
        ...
        if (sorted) {
            row += "Сортированный: ";
            row += reinterpret_cast<const char*>(sorted);
        }
...
// Тест производительности: добавление массивов
...
// Тест производительности: загрузка массивов
